...
export default function Results() {
  const { results, getResults, searchText, loading } = useStateContext()
  ...
  useEffect(() => {
    if(searchText !== ''){
      if(location.pathname === '/videos'){
        getResults(`/search/q=${searchText.split(' ').join('+')} videos`)
      } else{
        getResults(`/${location.pathname}/q=${searchText.split(' ').join('+')}&num=50`)
      }
    }
